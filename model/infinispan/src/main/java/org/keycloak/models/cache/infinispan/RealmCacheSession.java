...
public class RealmCacheSession implements CacheRealmProvider {
    ...
    // SP_POSITION
    protected Map<String, PositionAdapter> managedPositions = new HashMap<>();
    ...
    // SP_POSITION
    private PositionModel positionAdded(RealmModel realm, PositionModel position) {
        listInvalidations.add(realm.getId());
        invalidatePosition(position.getPosId(), realm.getId(), true);
        invalidationEvents.add(PositionAddedEvent.create(position.getPosId(), realm.getId()));
        return position;
    }

    @Override
    public PositionModel createPosition(RealmModel realm, PositionRepresentation rep) {
        PositionModel position = getRealmDelegate().createPosition(realm, rep);
        return positionAdded(realm, position);
    }
    @Override
    public List<PositionModel> searchForPositionByName(RealmModel realm, String search, Integer first, Integer max) {
        return getRealmDelegate().searchForPositionByName(realm, search, first, max);
    }

    @Override
    public List searchForPositionByNameAndCode(RealmModel realm, String searchName, String searchCode, Integer first, Integer max) {
        return getRealmDelegate().searchForPositionByNameAndCode(realm, searchName, searchCode, first, max);
    }

    @Override
    public List<Integer> checkPositionByNameAndCode(RealmModel realm, String searchName, String searchCode) {
        return getRealmDelegate().checkPositionByNameAndCode(realm, searchName, searchCode);
    }

    @Override
    public PositionModel getPositionById(String id, RealmModel realm) {
        if (id != null) {
            CachedPosition cached = cache.get(id, CachedPosition.class);
            if (cached != null && !cached.getRealm().equals(realm.getId())) {
                cached = null;
            }
            ...
            if (cached == null) {
                Long loaded = cache.getCurrentRevision(id);
                PositionModel model = getRealmDelegate().getPositionById(id, realm);
                if (model == null) return null;
                if (invalidations.contains(id)) return model;
                cached = new CachedPosition(loaded, realm, model);
                cache.addRevisioned(cached, startupRevision);

            } else if (invalidations.contains(id)) {
                return getRealmDelegate().getPositionById(id, realm);
            } else if (managedPositions.containsKey(id)) {
                return managedPositions.get(id);
            }
            PositionAdapter adapter = new PositionAdapter(cached, this, session, realm);
            managedPositions.put(id, adapter);
            return adapter;
        }
        return null;
    }
    ...
    @Override
    public void registerPositionInvalidation(String id) {
        invalidatePosition(id);
        addPositionEventIfAbsent(PositionUpdatedEvent.create(id));
    }
    private void invalidatePosition(String id, String realmId, boolean invalidateQueries) {
        invalidatePosition(id);
        if (invalidateQueries) {
            cache.positionQueriesInvalidations(realmId, invalidations);
        }
    }
    private void invalidatePosition(String id) {
        invalidations.add(id);
        PositionAdapter adapter = managedPositions.get(id);
        if (adapter != null) adapter.invalidate();
    }
    private void addPositionEventIfAbsent(InvalidationEvent eventToAdd) {
        String posId = eventToAdd.getId();
        // Check if we have existing event with bigger priority
        boolean eventAlreadyExists = invalidationEvents.stream()
                .anyMatch((InvalidationEvent event) -> (event.getId().equals(posId)) &&
                        (event instanceof PositionAddedEvent
//                                || event instanceof PositionMovedEvent || event instanceof PositionRemovedEvent
                        ));
        if (!eventAlreadyExists) {
            invalidationEvents.add(eventToAdd);
        }
    ...
    static String getPositionsQueryCacheKey(String realm) {
        return realm + ".positions";
    }

    @Override
    public boolean removePosition(RealmModel realm, PositionModel position) {
        invalidatePosition(position.getPosId(), realm.getId(), true);
        listInvalidations.add(realm.getId());
        cache.positionQueriesInvalidations(realm.getId(), invalidations);

        invalidationEvents.add(PositionRemovedEvent.create(position, realm.getId()));

        return getRealmDelegate().removePosition(realm, position);
    }
    @Override
    public List<PositionModel> getPositions(RealmModel realm) {
        String cacheKey = getPositionsQueryCacheKey(realm.getId());
        boolean queryDB = invalidations.contains(cacheKey) || listInvalidations.contains(realm.getId());
        if (queryDB) {
            return getRealmDelegate().getPositions(realm);
        }

        PositionListQuery query = cache.get(cacheKey, PositionListQuery.class);
        if (query != null) {
            logger.tracev("getPositions cache hit: {0}", realm.getName());
        }

        if (query == null) {
            Long loaded = cache.getCurrentRevision(cacheKey);
            List<PositionModel> model = getRealmDelegate().getPositions(realm);
            if (model == null) return null;
            Set<String> ids = new HashSet<>();
            for (PositionModel client : model) ids.add(client.getPosId());
            query = new PositionListQuery(loaded, cacheKey, realm, ids);
            logger.tracev("adding realm getPositions cache miss: realm {0} key {1}", realm.getName(), cacheKey);
            cache.addRevisioned(query, startupRevision);
            return model;
        }
        List<PositionModel> list = new LinkedList<>();
        for (String id : query.getPositions()) {
            PositionModel position = session.realms().getPositionById(id, realm);
            if (position == null) {
                invalidations.add(cacheKey);
                return getRealmDelegate().getPositions(realm);
            }
        ...
        list.sort(Comparator.comparing(PositionModel::getPosName));
