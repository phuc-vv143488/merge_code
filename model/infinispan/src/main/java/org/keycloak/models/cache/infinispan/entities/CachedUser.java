...
public class CachedUser extends AbstractExtendableRevisioned implements InRealm  {
    ...
    // SP_POSITION
    private final LazyLoader<UserModel, String> position;
    ...
    public CachedUser(Long revision, RealmModel realm, UserModel user, int notBefore) {
        ...
        // SP_POSITION
        this.position = new DefaultLazyLoader<>(userModel -> {
            PositionModel positionModel = userModel.getPosition();
            if (positionModel != null)
                return positionModel.getPosId();
            else return null;
        }, String::new);
    }
    ...
    // SP_POSITION
    public String getPosition(Supplier<UserModel> userModel) {
        return position.get(userModel);
    }
    // SP_POSITION
}
