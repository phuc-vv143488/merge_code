...
public class JpaUserProvider implements UserProvider, UserCredentialStore {
    ...
    public List<UserModel> searchForUser(Map<String, String> attributes, RealmModel realm, int firstResult, int maxResults) {
        ...
        // SP_POSITION
        String userPosition = (String) session.getAttribute(UserModel.POSITION);

        if (userPosition != null) {
            Subquery subquery = queryBuilder.subquery(String.class);
            Root<PositionEntity> from = subquery.from(PositionEntity.class);

            subquery.select(builder.literal(1));

            List<Predicate> subPredicates = new ArrayList<>();
            subPredicates.add(builder.equal(from.get("posId"), userPosition));
            ...
            Expression<String> positionId = from.get("posId");
            subs.add(builder.like(from1.get("name"), builder.concat("position.resource.", positionId)));
    ...
    // SP_POSITION
    @Override
    public void preRemove(RealmModel realm, PositionModel position) {
        em.createNamedQuery("deleteUserPositionMembershipsByPosition")
                .setParameter("positionId", position.getPosId())
                .setParameter("realmId", realm.getId())
                .executeUpdate();
    }
    @Override
    public List<UserModel> getPositionMembers(RealmModel realm, PositionModel position, int firstResult, int maxResults) {
        TypedQuery<UserEntity> query = em.createNamedQuery("positionMembership", UserEntity.class);
        query.setParameter("positionId", position.getPosId());
        if (firstResult != -1) {
            query.setFirstResult(firstResult);
        }
        if (maxResults != -1) {
    ...
    @Override
    public UserModel getPositionMember(RealmModel realm, PositionModel position, String userId) {
        TypedQuery<UserEntity> query = em.createNamedQuery("positionGetUser", UserEntity.class);
        query.setParameter("positionId", position.getPosId());
        query.setParameter("userId", userId);
        ...
        return new UserAdapter(session, realm, em, users.get(0));
    }
    // SP_POSITION
}
