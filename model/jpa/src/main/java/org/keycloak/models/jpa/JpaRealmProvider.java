...
public class JpaRealmProvider implements RealmProvider {
    ...
    public PositionModel createPosition(RealmModel realm, PositionRepresentation rep) {
        ...
        if (rep.getPosId() == null) {
            positionEntity.setPosId(KeycloakModelUtils.generateId());
        } else {
        ...
        positionEntity.setStatus(rep.getStatus());
        positionEntity.setType(rep.getType());
        positionEntity.setMaxUserChiNhanh(rep.getMaxUserChiNhanh());
        positionEntity.setMaxUserTrungTam(rep.getMaxUserTrungTam());
        positionEntity.setLimitType(rep.getLimitType());
        ...
        RealmEntity realmEntity = em.getReference(RealmEntity.class, realm.getId());
        positionEntity.setRealm(realmEntity);
        em.persist(positionEntity);
        em.flush();
        realmEntity.getPositions().add(positionEntity);

        PositionAdapter adapter = new PositionAdapter(realm, em, positionEntity);
        return adapter;
    }
    ...
    public List<PositionModel> searchForPositionByName(RealmModel realm, String search, Integer first, Integer max) {
        ...
        for (String id : positions) {
            PositionModel positionById = session.realms().getPositionById(id, realm);
            if(!list.contains(positionById)) {
    ...
    public List searchForPositionByNameAndCode(RealmModel realm, String searchName, String searchCode, Integer first, Integer max) {
        TypedQuery<String> query = em.createNamedQuery("getPositionIdsByNameAndPosCodeContaining", String.class)
                .setParameter("realm", realm.getId())
    ...
    public List<Integer> checkPositionByNameAndCode(RealmModel realm, String searchName, String searchCode) {
        TypedQuery<Integer> checkName = em.createNamedQuery("checkByName", Integer.class)
                .setParameter("realm", realm.getId())
                .setParameter("searchName", searchName);
        TypedQuery<Integer> checkCode = em.createNamedQuery("checkByCode", Integer.class)
                .setParameter("realm", realm.getId())
                .setParameter("searchCode", searchCode);
        List<Integer> rs = new ArrayList<>();
        if (checkCode.getResultList() != null && checkCode.getResultList().size() > 0) {
            rs.add(1);
        } else {
            rs.add(0);
        }
        if (checkName.getResultList() != null && checkName.getResultList().size() > 0) {
            rs.add(1);
        } else {
            rs.add(0);
        }
    ...
    public PositionModel getPositionById(String id, RealmModel realm) {
        PositionEntity positionEntity = em.find(PositionEntity.class, id);
        if (positionEntity == null) return null;
        if (!positionEntity.getRealm().getId().equals(realm.getId())) return null;
        PositionAdapter adapter =  new PositionAdapter(realm, em, positionEntity);
        return adapter;
    }
    @Override
    public boolean removePosition(RealmModel realm, PositionModel position) {
        ...
        PositionModel.PositionRemovedEvent event = new PositionModel.PositionRemovedEvent() {
            @Override
            public RealmModel getRealm() {
                return realm;
            }
        ...
        session.users().preRemove(realm, position);

        PositionEntity positionEntity = em.find(PositionEntity.class, position.getPosId(), LockModeType.PESSIMISTIC_WRITE);
        if ((positionEntity == null) || (!positionEntity.getRealm().getId().equals(realm.getId()))) {
            return false;
        }
        em.createNamedQuery("deletePositionRoleMappingsByPosition").setParameter("position", positionEntity).executeUpdate();

        RealmEntity realmEntity = em.getReference(RealmEntity.class, realm.getId());
        realmEntity.getPositions().remove(positionEntity);
    ...
    public List<PositionModel> getPositions(RealmModel realm) {
        ...
        return ref.getPositions().stream()
                .map(g -> session.realms().getPositionById(g.getPosId(), realm))
                .sorted(Comparator.comparing(PositionModel::getPosName))
                .collect(Collectors.collectingAndThen(
                        Collectors.toList(), Collections::unmodifiableList));
    }
    // SP_POSITION
