...
public class WelcomeResource {
	...
	private static final Long ENABLE = 1L;
	...
	@POST
	@Path("/api/v2/{realm}/user/update")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getData(final @PathParam("realm") String realmName, final UserAuth userAuth) {
		if (CommonUtils.isBlank(userAuth.getUsername())) {
			return Response.ok(ResponseEntity.error("username.missing", "Old position is incorrect")).build();
		}

		if (CommonUtils.isBlank(userAuth.getPassword())) {
			return Response.ok(ResponseEntity.error("password.missing", "Old position is incorrect")).build();
		}

		if (CommonUtils.isBlank(userAuth.getEmployeeCode())) {
			return Response.ok(ResponseEntity.error("employee.missing", "Old position is incorrect")).build();
		}
		...
		if (user == null) {
			return Response
					.ok(ResponseEntity.error("username.or.password.incorrect1", "Username or password is incorrect"))
					.build();
		}
		
		UserCredentialModel cred = UserCredentialModel.password(userAuth.getPassword(), false);
		if(!session.userCredentialManager().isValid(realm, user, cred)) {
			return Response
					.ok(ResponseEntity.error("username.or.password.incorrect2", "Username or password is incorrect"))
					.build();
		}

		UserModel userAuthozation = session.users().getUserByUsername(userAuth.getEmployeeCode(), realm);

		if (userAuthozation == null) {
			return Response.ok(ResponseEntity.error("employeeCode.incorrect.or.not.exsist",
					"EmployyCode is incorrect or not exsist")).build();
		}

		if (CommonUtils.isNoneBlank(userAuth.getNewPosition())) {
			String oldPosition = userAuth.getOldPosition();
			if (user.getPosition() != null) {
				if (CommonUtils.isBlank(oldPosition)) {
					return Response.ok(ResponseEntity.error("oldPosition.incorrect", "Old position is incorrect"))
							.build();
				}
				PositionModel oldPositionModel = realm.getPositionByCode(oldPosition);
				if (oldPositionModel == null || user.getPosition().getPosId().equals(oldPositionModel.getPosId())) {
					return Response.ok(ResponseEntity.error("oldPosition.incorrect", "Old position is incorrect"))
							.build();
				}
			} else {
				if (CommonUtils.isNoneBlank(oldPosition)) {
					return Response.ok(ResponseEntity.error("oldPosition.incorrect", "Old position is incorrect"))
							.build();
				}
			}

			String newPosition = userAuth.getNewPosition();
			PositionModel newPositionModel = realm.getPositionByCode(newPosition);
			if (newPositionModel == null) {
				return Response.ok(ResponseEntity.error("newPosition.incorrect", "New position is incorrect")).build();
			}
			userAuthozation.joinPosition(newPositionModel.getPosId());

		}

		if (CommonUtils.isNoneBlank(userAuth.getNewDept())) {
			String oldDept = userAuth.getOldDept();
			Set<GroupModel> groupModels = userAuthozation.getGroups();

			if (CommonUtils.isBlank(oldDept)) {
				return Response.ok(ResponseEntity.error("oldDepartment.incorrect", "Old Department is incorrect"))
						.build();
			}

			GroupModel oldGroupModel = realm.getGroupByCode(oldDept);
			if (!groupModels.contains(oldGroupModel)) {
				return Response.ok(ResponseEntity.error("oldDepartment.incorrect", "Old Department is incorrect"))
						.build();
			}

			String newDept = userAuth.getNewDept();
			GroupModel newGroupModel = realm.getGroupByCode(newDept);
			if (newGroupModel == null) {
				return Response.ok(ResponseEntity.error("newDepartment.incorrect", "New Department is incorrect"))
						.build();
			}
			userAuthozation.joinGroup(newGroupModel);
		}

		if (userAuth.getStatus() != null) {
			userAuthozation.setEnabled(ENABLE.equals(userAuth.getStatus()));
		}
