...
public class UserStorageManager implements UserProvider, OnUserCache, OnCreateComponent, OnUpdateComponent {
    ...
    public void preRemove(RealmModel realm, PositionModel position) {
        localStorage().preRemove(realm, position);
        if (getFederatedStorage() != null) {
            getFederatedStorage().preRemove(realm, position);
            for (UserStorageProvider provider : getEnabledStorageProviders(session, realm, UserStorageProvider.class)) {
                provider.preRemove(realm, position);
            }
        }
    }
    @Override
    public List<UserModel> getPositionMembers(final RealmModel realm, final PositionModel position, int firstResult, int maxResults) {
        List<UserModel> results = query((provider, first, max) -> {
            if (provider instanceof UserQueryProvider) {
                return ((UserQueryProvider)provider).getPositionMembers(realm, position, first, max);
    ...
    @Override
    public UserModel getPositionMember(final RealmModel realm, final PositionModel position, String userId) {
        List<UserModel> results = query((provider, first, max) -> {
            if (provider instanceof UserQueryProvider) {
                UserModel user = ((UserQueryProvider)provider).getPositionMember(realm, position, userId);
                List<UserModel> lst = new LinkedList<UserModel>();
                if(user != null) {
                    lst.add(user);
                }
                return lst;
        ...
        List<UserModel> lstRs = importValidation(realm, results);
