...
public class ModelToRepresentation {
    ...
    public static UserRepresentation toRepresentation(KeycloakSession session, RealmModel realm, UserModel user) {
        ...
        // SP_POSITION
        rep.setPositionId(getPositionId(user));
        // SP_POSITION
    ...
    public static ResourceRepresentation toRepresentation(Resource model, ResourceServer resourceServer, AuthorizationProvider authorization, Boolean deep) {
        ...
        if (model.getType().equalsIgnoreCase(MENU)) {
        	resource.setParentId(model.getParentId());
            resource.setOrder(model.getOrder());
            resource.setHaveChild(model.getHaveChild());
        }
    ...
    // SP_POSITION
    public static PositionRepresentation toRepresentation(PositionModel position, boolean full) {
        PositionRepresentation rep = new PositionRepresentation();
        rep.setPosId(position.getPosId());
        rep.setPosName(position.getPosName());
        rep.setPosCode(position.getPosCode());
        rep.setDescription(position.getDescription());
        rep.setStatus(position.getStatus());
        rep.setType(position.getType());
        rep.setMaxUserChiNhanh(position.getMaxUserChiNhanh());
        rep.setMaxUserTrungTam(position.getMaxUserTrungTam());
        rep.setLimitType(position.getLimitType());
        rep.setValidDateStart(position.getValidDateStart());
        rep.setValidDateEnd(position.getValidDateEnd());
        rep.setCreateDate(position.getCreateDate());

        if (!full) return rep;
        // Role mappings
        Set<RoleModel> roles = position.getRoleMappings();
        List<String> realmRoleNames = new ArrayList<>();
        Map<String, List<String>> clientRoleNames = new HashMap<>();
        for (RoleModel role : roles) {
            if (role.getContainer() instanceof RealmModel) {
        ...
        rep.setRealmRoles(realmRoleNames);
        rep.setClientRoles(clientRoleNames);
        Map<String, List<String>> attributes = position.getAttributes();
...
//    public static List<PositionRepresentation> toPositionHierarchy(UserModel user, boolean full, Integer first, Integer max) {
//        return user.getPositions(first, max).stream()
//                .map(position -> toRepresentation(position, full))
//                .collect(Collectors.toList());
//    }
//
//    public static PositionRepresentation toPositionHierarchy(UserModel user, boolean full) {
//        return toRepresentation(user.getPosition(), full);
//    }
//
//    public static List<PositionRepresentation> searchForPositionByName(UserModel user, boolean full, String search, Integer first, Integer max) {
//        return user.getPositions(search, first, max).stream()
//                .map(position -> toRepresentation(position, full))
//                .collect(Collectors.toList());
//    }
//
//    public static List<PositionRepresentation> searchForPositionByName(RealmModel realm, boolean full, String search, Integer first, Integer max) {
//        List<PositionRepresentation> result = new LinkedList<>();
//        List<PositionModel> positions = realm.searchForPositionByName(search, first, max);
//        if (Objects.isNull(positions)) return result;
//        for (PositionModel position : positions) {
//            PositionRepresentation rep = toRepresentation(position, full);
//            result.add(rep);
//        }
//        return result;
//    }
    ...
    private static String getPositionId(UserModel user) {
        if (user != null) {
