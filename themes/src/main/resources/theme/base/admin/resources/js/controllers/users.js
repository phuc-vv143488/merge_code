...
module.controller('UserDetailCtrl', function($scope, realm, user, BruteForceUser, User,
    ...
    // SP_POSITION
    const convertPositionOfUser = function(user) {
        if (!user.positionId) {
    ...
    /*
    * SP_POSITION : GET LIST
    * */
    Position.query({realm: realm.realm, first : 0}, function (data) {
        $scope.positionList = [];
        for (let i = 0; i < data.length; i++) {
            $scope.positionList.push(data[i]);
        }
    });
    // SP_POSITION
    ...
    $scope.save = function() {
        ...
        convertPositionOfUser($scope.user);
...
// SP_POSITION
module.controller('UserPositionMembershipCtrl', function($scope, $q, PositionSearchState, PositionMemberSearchState, realm, user, UserPositionMembership, UserPositionMapping, Notifications, Position) {
    $scope.queryMembers = {};
    $scope.query = {};
    $scope.all = {};
    $scope.members = {};

    $scope.allPositionMemberships = []; // ALL OF USER
    $scope.pagingPositionMemberships = []; // Paging OF USER
    $scope.positionList = []; // ALL POSITION
    // let checkFirstQueryMember = true;

    $scope.init = function () {
        $scope.realm = realm;
        $scope.user = user;
        PositionSearchState.query.realm = realm.realm;
        PositionMemberSearchState.query.realm = realm.realm;
        $scope.query = PositionSearchState.query;
        $scope.queryMembers = PositionMemberSearchState.query;
        $scope.queryMembers.userId = user.id;

        refreshCompleteUserPositionMembership();


        // $scope.searchQueryMembers();
        if (!PositionMemberSearchState.test) {
            searchQueryMembers();
            console.log('PositionMemberSearchState')
        }

        if (!PositionSearchState.isFirstSearch) {
            searchQuery();
            console.log('PositionSearchState')
        }
    };
    $scope.all.searchLoaded = false;
    $scope.all.lastSearch = false;

    $scope.members.searchLoaded = false;
    $scope.members.lastSearch = false;

    $scope.firstPageAll = function () {
        $scope.query.first = 0;
        searchQuery();
    };
    $scope.previousPage = function () {
    ...
    const searchQuery = function () {
        $scope.all.searchLoaded = false;
        $scope.positionList = Position.query($scope.query, function () {
            $scope.all.searchLoaded = true;
            $scope.all.lastSearch = $scope.query.search;
            PositionSearchState.isFirstSearch = false;
        });
    };
    $scope.firstPageMember = function () {
        $scope.queryMembers.first = 0;
        searchQueryMembers();
    };
    $scope.previousPageMember = function () {
        $scope.queryMembers.first -= parseInt($scope.query.max);
        if ($scope.query.first < 0) {
            $scope.query.first = 0;
        }
        searchQueryMembers();
    ...
    const searchQueryMembers = function() {
        $scope.members.searchLoaded = false;
        $scope.pagingPositionMemberships = UserPositionMembership.query($scope.queryMembers, function () {
            $scope.members.searchLoaded = true;
            $scope.members.lastSearch = $scope.queryMembers.search;
            PositionMemberSearchState.test = false;
        });
    };
    $scope.joinPosition = function(position) {
        if (!position) {
            Notifications.error('Please select a position to add');
            return;
        }
        if (isMember(position)) {
            Notifications.error('Position already added');
            return;
        }
        UserPositionMapping.update({realm: realm.realm, userId: user.id, posId: position.posId}, function() {
            $scope.allPositionMemberships.push(position);
            // refreshUserPositionMembership($scope.searchCriteriaMembership);
            searchQueryMembers();
            Notifications.success('Added position membership');
        });

    };

    $scope.leavePosition = function(position) {
        if (!position) {
            Notifications.error('Please select a position to remove');
            return;
        }
        UserPositionMapping.remove({realm: realm.realm, userId: user.id, posId: position.posId}, function () {
            removePositionMember($scope.allPositionMemberships, position);
            // refreshUserPositionMembership($scope.searchCriteriaMembership);
            searchQueryMembers();
            Notifications.success('Removed position membership');
        });

    };
    const refreshCompleteUserPositionMembership = function() {
        var queryParams = {
            realm : realm.realm,
            userId: user.id
        };
        ...
        console.log('refresh');
        var promiseGetCompleteUserPositionMembership = $q.defer();
        UserPositionMembership.query(queryParams, function(entry) {
            promiseGetCompleteUserPositionMembership.resolve(entry);
        }, function() {
            promiseGetCompleteUserPositionMembership.reject('Unable to fetch all position memberships' + queryParams);
        });
        promiseGetCompleteUserPositionMembership.promise.then(function(positions) {
            for (let i = 0; i < positions.length; i++) {
                $scope.allPositionMemberships.push(positions[i]);
                // $scope.getPositionClass(positions[i]);
            }
        }, function (failed) {
            Notifications.error(failed);
        });
    ...
    const isMember = function(position) {
        for (var i = 0; i < $scope.allPositionMemberships.length; i++) {
            var member = $scope.allPositionMemberships[i];
            if (position.posId === member.posId) {
                return true;
            }
        }
        return false;
    };
    $scope.isMember = function(position) {
        for (var i = 0; i < $scope.allPositionMemberships.length; i++) {
            var member = $scope.allPositionMemberships[i];
            if (position.posId === member.posId) {
                return true;
...
function removePositionMember(positions, member) {
    for (var j = 0; j < positions.length; j++) {
        if (member.posId === positions[j].posId) {
