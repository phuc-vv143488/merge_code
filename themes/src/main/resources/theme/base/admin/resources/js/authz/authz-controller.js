...
module.controller('MenuDetailCtrl', function(Dialog, $scope, client, realm, group, Group, Notifications, $location) {
    $scope.realm = realm;
    $scope.client = client;
    if (!group.attributes) {
        group.attributes = {}
...
module.controller('MenuCreateCtrl', function($scope, $route, realm, parentId, client, Groups, Group, GroupChildren, Notifications, $location) {
    $scope.realm = realm;
    ...
    $scope.save = function() {
        ...
        if (parentId === 'realm') {
            ...
            Groups.save({realm: realm.realm}, $scope.group, function(data, headers) {
                ...
                $location.url("/realms/" + realm.realm + "/clients/" + client.id + "/authz/resource-server/menu/groups/" + id);
...
module.controller('MenuListCtrl', function($scope,ResourceServerResource, $route, $q, realm, Groups, GroupsCount, client, Group, GroupChildren, Notifications, $location, Dialog, ComponentUtils) {
    $scope.realm = realm;
    $scope.client = client;
    ...
    var refreshGroups = function (search) {
        ...
        promiseGetGroups.promise.then(function(groups) {
        	...
        	console.log('stringObj new:'+stringObj);
    ...
    $scope.edit = function(selected) {
        ...
        $location.url("/realms/" + realm.realm + "/clients/" + $scope.client.id + "/authz/resource-server/menu/" + selected.id);
    };
    ...
    $scope.remove = function(selected) {
        ...
        Dialog.confirmDelete(selected.name, 'group', function() {
            ResourceServerResource.delete({realm : $route.current.params.realm, client : $scope.client.id, rsrid : selected.id}, null, function() {
               // $location.url("/realms/" + $route.current.params.realm + "/clients/" + $scope.client.id + "/authz/resource-server/menu");
                $route.reload();
                Notifications.success("The menu has been deleted.");
            });
        });
    ...
    $scope.createGroup = function(selected) {
        ...
        $location.url("/realms/" + realm.realm + "/clients/" + client.id + "/authz/resource-server/menu/create/parent/" + parent);
...
module.controller('MenuResourceCreateCtrl', function($scope, $http, $route, $location, realm, ResourceServer, client, parentId, ResourceServerResource, ResourceServerScope, AuthzDialog, Notifications) {
    $scope.realm = realm;
    $scope.client = client;
    ...
    ResourceServer.get({
        ...
        if (!resourceId) {
            ...
            var resource = {};
            resource.scopes = [];
            //resource.attributes = {parentId:[parentId]};
            resource.uris = [];
            ...
            $scope.save = function() {
                ...
                console.log('log at save 1');
                $scope.resource.type='menu';
                $scope.resource.parentId=parentId;
                $instance.checkNameAvailability(function () {
                    ResourceServerResource.save({realm : realm.realm, client : $scope.client.id}, $scope.resource, function(data) {
                        $location.url("/realms/" + realm.realm + "/clients/" + $scope.client.id + "/authz/resource-server/menu/" + data._id);
    ...
    ResourceServer.get({
        ...
        if (!resourceId) {
            ...
            $scope.save = function() {
            	...
            	if ($scope.resource.type != 'menu') {
                	$scope.resource.type='resource';
            	}
                if ($scope.newUri && $scope.newUri.length > 0) {
                    $scope.addUri();
                }
...
// SP_POSITION
module.controller('ResourceServerPolicyPositionDetailCtrl', function($scope, $route, realm, client, Client, Position, PolicyController, Notifications) {
    PolicyController.onInit({
        getPolicyType : function() {
            return "position";
        },
        ...
        onInit : function() {
            ...
            $scope.selectPosition = function(position) {
                if (!position || !position.posId) {
                    return;
                }
                $scope.selectedPosition = null;

                for (let i = 0; i < $scope.selectedPositions.length; i++) {
                    if ($scope.selectedPositions[i].posId == position.posId) {
                        return;
                    }
                }

                if (position.posId == "realm") {
                    Notifications.error("You must choose a position");
                    return;
                }
            ...
            // $scope.checkChanged = function(selectedPositions){
            //     var index = $scope.selectedPositions.length
            //     if (index = 0){
            //         $scope.changed = true;
            //     }
            // }
            ...
            $scope.checkErr = function (selectedPositions) {
                $scope.errMessage = '';
                if (selectedPositions) {
                    if ($scope.selectedPositions && $scope.selectedPositions.length === 0) {
            ...
            $scope.removeFromList = function(position) {

                var index = $scope.selectedPositions.indexOf(position);
                if (index != -1) {
                    $scope.selectedPositions.splice(index, 1);
                    $scope.changed = true;
                }
                const isRequire = $scope.selectedPositions.length
                console.log('$scope.selectedPositions', isRequire, $scope.selectedPositions)

                console.log('position', position);
                if ($scope.selectedPositions.length == 0 || isRequire == 1) {
                    $scope.changed = false;
                }
            }
        },
        ...
        onInitCreate : function(policy) {
            ...
            $scope.selectedPositions = angular.copy(selectedPositions);

            $scope.$watch('selectedPositions', function() {
                if (!angular.equals($scope.selectedPositions, selectedPositions)) {
                    $scope.changed = true;
                } else {
                    $scope.changed = PolicyController.isNewAssociatedPolicy();
                }
        ...
        onInitUpdate : function(policy) {
            $scope.changed = false;
            $scope.selectedPositions = policy.positions;

            angular.forEach($scope.selectedPositions, function(position, index){
                Position.get({realm: $route.current.params.realm, posId: position.posId}, function (existing) {
                    position.posName = existing.posName;
                });
            });
        ...
        onUpdate : function() {
            $scope.policy.positions = $scope.selectedPositions;
            delete $scope.policy.config;
        },
    ...
    $scope.hasRealmPosition = function () {
        for (i = 0; i < $scope.selectedPositions.length; i++) {
            if (!$scope.selectedPositions[i].clientPosition) {
                return true;
            }
        }
        return false;
