...
module.controller('DomainListCtrl', function($scope, $route, $q, realm, Domains, GroupsCount, Group, GroupChildren, Notifications, $location, Dialog, ComponentUtils) {
    $scope.realm = realm;
    $scope.groupList = [
        {
            "id" : "realm",
    ...
    $scope.edit = function(selected) {
        if (selected.id === 'realm') return;
        console.log('code:'+selected.code);
        $location.url("/realms/" + realm.realm + "/domains/" + selected.id);
    };
    ...
    $scope.paste = function(selected) {
        ...
        if (selected.id === $scope.cutNode.id) return;
        if (selected.id === 'realm') {
            Domains.save({realm: realm.realm}, {id:$scope.cutNode.id}, function() {
                $route.reload();
                Notifications.success("Group moved.");
    ...
    $scope.createGroup = function(selected) {
        ...
        console.log('parentId:'+parent);
        $location.url("/create/domain/" + realm.realm + '/parent/' + parent);
...
module.controller('DomainCreateCtrl', function($scope, $route, realm, parentId, Groups, Group, GroupChildren, Notifications, $location) {
    $scope.realm = realm;
    $scope.group = {};
    $scope.group.domain = true;
    $scope.save = function() {
        console.log('save!!!');
        if (parentId === 'realm') {
            console.log('realm');
            Groups.save({realm: realm.realm}, $scope.group, function(data, headers) {
                ...
                $location.url("/realms/" + realm.realm + "/domains/" + id);
                Notifications.success("Group Created.");
            })
    ...
    $scope.cancel = function() {
        $location.url("/realms/" + realm.realm + "/domains");
    };
});
...
module.controller('DomainDetailCtrl', function(Dialog, $scope, realm, group, Group, Notifications, $location) {
    $scope.realm = realm;
...
module.controller('DomainMembersCtrl', function($scope, realm, group, GroupMembership) {
    $scope.realm = realm;
    $scope.page = 0;
    $scope.group = group;
